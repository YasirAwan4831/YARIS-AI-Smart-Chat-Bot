...
const App: React.FC = () => {
  ...
  const handleSendMessage = async (text: string) => {
    ...
    const userMessage: Message = { id: generateId(), role: Role.USER, text, timestamp: Date.now() };
    ...
    setConversations((prev) =>
      prev.map((c) =>
        c.id === activeConversationId
          ? { ...c, messages: [...c.messages, { id: responseId, role: Role.MODEL, text: '', timestamp: Date.now() }] }
          : c
      )
    );
    ...
    try {
      ...
      await generateChatResponseStream(history, text, (chunk) => {
        ...
        setConversations((prev) =>
          prev.map((c) =>
            c.id === activeConversationId
              ? { ...c, messages: c.messages.map((msg) => (msg.id === responseId ? { ...msg, text: currentResponseText } : msg)) }
              : c
          )
        );
      });
          ...
          c.id === activeConversationId
            ? {
                ...
                messages: c.messages.map((msg) =>
                  msg.id === responseId
                    ? { ...msg, text: 'Error connecting to the service. Please try again.', isError: true }
                    : msg
                ),
              }
            : c
  ...
  return (
    <div className="flex h-screen bg-background font-sans">
      ...
      <main className="flex-1 flex flex-col h-full relative min-w-0">
        ...
        <div ref={chatContainerRef} className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6">
        ...
        <InputArea onSendMessage={handleSendMessage} disabled={isTyping || !activeConversationId} />
